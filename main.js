...
const projectsData = [
  ...
  },
  {
    ...
    description:
      "用Vue3和Go搭建的微软New Bing演示站点，拥有一致的UI体验，支持ChatGPT提示词，支持API调用，国内可用。",
    tags: ["HTML", "Vue", "Go"],
    stars: 0,
    forks: 0,
  },
...
function parseRSS(xmlText) {
  ...
  // 需要过滤掉的页面标题
  const filteredTitles = ["友链", "关于", "导航"];

  items.forEach((item, index) => {
    const title = item.querySelector("title")?.textContent || "无标题";

    // 过滤掉指定标题的页面
    if (filteredTitles.includes(title.trim())) {
      return;
    }
...
async function fetchArticlesData() {
  try {
    ...
    // 检测是否为 RSS 源（以 .xml 结尾或包含 rss）
    const isRSS =
      ARTICLES_API_CONFIG.URL.includes(".xml") ||
      ARTICLES_API_CONFIG.URL.includes("rss") ||
      ARTICLES_API_CONFIG.URL.includes("feed");
    ...
    const sortedRows = articles.sort(
  ...
  try {
    ...
    if (mostRecentRepo) {
      ...
      const daysSinceLastCommit =
        (new Date() - new Date(mostRecentRepo.last_commit)) /
        (1000 * 60 * 60 * 24);
      ongoingProject.progress = Math.max(
        0,
        Math.min(100, 100 - Math.floor(daysSinceLastCommit)),
      );
    }
      ...
      if (eventDate >= startDate && eventDate <= endDate) {
        ...
        switch (event.type) {
          ...
          case "WatchEvent":
            ...
            const existingWatchActivity = activitiesByMonth[monthStr].find(
              (activity) =>
                activity.type === "star" &&
      ...
      if (commitDate >= startDate && commitDate <= endDate) {
        ...
        if (!existingActivity) {
          ...
          if (
            ...
            new Date(existingActivity.created_at || 0)
          ) {
      ...
      if (starDate >= startDate && starDate <= endDate) {
        ...
        if (!existingActivity) {
          ...
          activitiesByMonth[monthStr].push({
            ...
            date: starDate.toLocaleString("en-US", {
              month: "short",
              day: "numeric",
            }),
        ...
        if (!existingActivity) {
          ...
          activitiesByMonth[monthStr].push({
            ...
            description: fork.originalRepo
              ...
              : `Forked ${fork.repo}`,
            date: forkDate.toLocaleString("en-US", {
              month: "short",
              day: "numeric",
            }),
...
async function refreshContributionData() {
  try {
    ...
    // 强制刷新当前选择年份的数据
    const { contributionData: data, activityData } =
      await fetchContributionData(yearToRefresh, true);
...
function renderActivityTimeline(data, activities = null) {
  ...
  sortedMonths.forEach((month) => {
    ...
    if (sortedMonthActivities.length === 0) {
      ...
      monthContentHtml = sortedMonthActivities
        .map((activity) => {
          // 跳过 star 类型的活动
          ...
          const activityItemClass = shouldBeVisible
            ? ""
            : "hidden-activity-item";
                            ...
                            <div class="activity-item ${activityItemClass}">
                                ...
                                <div class="activity-content">
                                    ...
                                    ${
                                      ...
                                      activity.commitMessages.length > 0
                                        ? `
                                    ...
                                    ${
                                      ...
                                      "hidden-activity-item"
                                        ? (() => {
                                            ...
                                            const repoInfo = projectsData.find(
                                              (project) =>
                                                project.name === activity.repo,
                                            );
                                            return repoInfo &&
  ...
  if (totalActivityItems > initialVisibleCount) {
    ...
    const actualRemainingItems = Math.min(
      totalActivityItems - initialVisibleCount,
      maxRemainingItems,
    );
...
function calculateCodeReviewStats(activityData) {
  ...
  // 计算总数（避免除零）
  const totalCount =
    totalCommits + totalIssues + totalPullRequests + totalReviews;
  ...
  const pullRequestsPercentage = Math.round(
    (totalPullRequests / totalCount) * 100,
  );
...
async function renderContributionGraph() {
  ...
  // 确保选择的年份在有效范围内
  const yearToUse = validYears.includes(selectedYear)
    ? selectedYear
    : currentYear;

  const { contributionData: data, activityData } =
    await fetchContributionData(yearToUse);
  const totalContributions = data.reduce(
    (sum, day) => sum + day.contributions,
    0,
  );
  ...
  // 动态生成年份选项的HTML
  const yearOptionsHTML = validYears
    .map(
      (year) =>
            ...
            <div class="contribution-calendar">
                ...
                <div class="contribution-legend">
                    ...
                    ${[0, 1, 2, 3, 4]
                      .map(
                        (level) =>
                          `<div class="legend-item" data-level="${level}"></div>`,
                      )
                      .join("")}
                    <span>More</span>
                </div>
...
async function updateContributionYear(year, forceRefresh = false) {
  try {
    ...
    // 重新获取并渲染贡献数据
    const { contributionData: data, activityData } =
      await fetchContributionData(year, forceRefresh);
    const totalContributions = data.reduce(
      (sum, day) => sum + day.contributions,
      0,
    );
    ...
    if (codeReviewChart) {
      ...
      // 清除旧的百分比元素并添加新的
      const oldPercentages = codeReviewChart.querySelectorAll(
...
async function renderOverview() {
  ...
  try {
    ...
    [
      ...
      "C++",
      "C#",
    ].forEach((lang) => allLanguages.add(lang));
...
async function renderProjects() {
  ...
  try {
    ...
    // 渲染所有项目
    content.innerHTML = repoDetails
      .map((repo) => renderProjectCard(repo))
      .join("");
  } catch (error) {
...
function showQQ(event) {
  ...
  const qrCodeUrl = isDarkMode
    ? "https://cdn.rjjr.cn/avatar/IMG_0233.JPG"
    : "https://cdn.rjjr.cn/assets/e1f67108d1e5ab51216cee6f503b878d.png";
  showQRCode(
    "QQ",
    qrCodeUrl,
    "直接点击此处加好友",
    "https://qm.qq.com/q/W2oxo3R3qy",
  );
}
...
async function fetchStarredRepos() {
  try {
    ...
    const response = await fetch(
      `${GITHUB_API_BASE}/users/${GITHUB_USERNAME}/starred`,
      {
        headers: headers,
      },
    );
...
async function fetchProjectDetail(projectName) {
  try {
    ...
    const [owner, repo] = projectName.includes("/")
      ? projectName.split("/")
      : [GITHUB_USERNAME, projectName];
    ...
    const repoResponse = await fetch(
      `${GITHUB_API_BASE}/repos/${cleanOwner}/${cleanRepo}`,
      { headers },
    );
    if (!repoResponse.ok) {
    ...
    try {
      const commitsResponse = await fetch(
        `${GITHUB_API_BASE}/repos/${cleanOwner}/${cleanRepo}/commits?per_page=5`,
        { headers },
      );
      if (commitsResponse.ok) {
...
async function showProjectDetail(projectName) {
  ...
  try {
    ...
    // 计算语言百分比
    const totalBytes = Object.values(projectData.languages).reduce(
      (a, b) => a + b,
      0,
    );
                        ...
                        <div class="project-languages-section">
                            ...
                            <div class="language-list">
                                ${languagesWithPercentage
                                  .map((lang) => {
                                    const langNameLower = normalizeLangName(
                                      lang.name,
                                    );
                                    return `
...
function toggleActivity(event) {
  ...
  if (hiddenItems.length > 0) {
    ...
    if (actualRemainingItems > 0) {
      ...
      requestAnimationFrame(() => {
        ...
        requestAnimationFrame(() => {
          ...
          if (lastNewItem) {
            ...
            const targetY =
              window.pageYOffset + rect.top - window.innerHeight / 2; // 居中显示
      ...
      requestAnimationFrame(() => {
        if (
          currentVisibleActivityCount >= maxActivityItems ||
          currentVisibleActivityCount - 5 >= totalHiddenItems
        ) {
          ...
          // 还有更多项目可以显示
          const nextRemainingItems = Math.min(
            remainingItems - itemsToShow,
            maxActivityItems - currentVisibleActivityCount,
          );
  ...
  if (hiddenItems.length > 0) {
    ...
    if (currentVisibleActivityCount > 5) {
      ...
      if (isAtBottom) {
        ...
        // 收起最近展开的部分
        const itemsToHide = Math.min(
          activityIncrement,
          currentVisibleActivityCount - 5,
        );
        const startIndex = Math.max(
          0,
          currentVisibleActivityCount - 5 - itemsToHide,
        );
        ...
        // 先更新计数器，确保状态一致
        currentVisibleActivityCount = Math.max(
          5,
          currentVisibleActivityCount - itemsToHide,
        );
      ...
      if (!isAtBottom) {
        const remainingItems =
          totalHiddenItems - (currentVisibleActivityCount - 5);
...
// 获取说说数据
async function fetchTgtalkData() {
  try {
    ...
    if (!data.success) {
      throw new Error("Tgtalk API returned unsuccessful response");
    }
...
// 格式化说说时间（时间戳转相对时间）
function formatTgtalkTime(timestamp) {
  const date = new Date(timestamp);
  return timeAgo(date);
}

// 处理说说内容（支持 HTML 和 Markdown）
function processTgtalkContent(text) {
  if (!text) return "";
...
// 创建说说卡片 HTML
function createTgtalkHTML(item) {
      ...
      <div class="memo-header">
        ...
        <span class="memo-views">
          <svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor">
            <path d="M8 2c1.981 0 3.671.992 4.933 2.078 1.27 1.091 2.187 2.345 2.637 3.023a1.62 1.62 0 0 1 0 1.798c-.45.678-1.367 1.932-2.637 3.023C11.67 13.008 9.981 14 8 14c-1.981 0-3.671-.992-4.933-2.078C1.797 10.83.88 9.576.43 8.898a1.62 1.62 0 0 1 0-1.798c.45-.678 1.367-1.932 2.637-3.023C4.33 2.992 6.019 2 8 2ZM1.679 7.932a.25.25 0 0 0 0 .136c.305.461.778 1.06 1.39 1.584C3.831 10.297 5.668 11.5 8 11.5c2.332 0 4.169-1.203 4.931-1.948.612-.524 1.085-1.123 1.39-1.584a.25.25 0 0 0 0-.136c-.305-.461-.778-1.06-1.39-1.584C12.169 4.703 10.332 3.5 8 3.5c-2.332 0-4.169 1.203-4.931 1.948-.612.524-1.085 1.123-1.39 1.584Z"/>
            <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5ZM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0Z"/>
          </svg>
          ${item.views || 0}
        </span>
      </div>
      <div class="memo-content tgtalk-content">
        ${processedContent}
        ${imagesHTML}
      </div>
    </div>
...
async function renderTgtalk() {
  try {
    ...
    console.error("Error rendering tgtalk:", error);
    const contentArea = document.querySelector(".content-area");
    contentArea.innerHTML = `
      <div class="blankslate">
        <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
function createMemoHTML(memo) {
  // 兼容 v0.26.0 新格式：使用 displayTime 或 createTime
  const timeField = memo.displayTime || memo.createTime;
  ...
  // 处理资源（图片等）- 兼容 v0.26.0 新格式
  let resourcesHTML = "";
  if (memo.resources && memo.resources.length > 0) {
    resourcesHTML = `<div class="memo-resources">${memo.resources
      .map((resource) => {
        // v0.26.0 使用 name 字段，旧版使用 id
        const resourceId = resource.name || resource.id;
        const memosBaseUrl = MEMOS_API_BASE.replace("/api/v1", "");
        const imageUrl =
          resource.externalLink ||
          resource.url ||
          `${memosBaseUrl}/o/r/${resourceId}`;
        return `<img src="${imageUrl}" alt="资源图片" class="memo-resource">`;
      })
      .join("")}</div>`;
  }
  // 兼容 v0.26.0 的 attachments 字段
  else if (memo.attachments && memo.attachments.length > 0) {
    resourcesHTML = `<div class="memo-resources">${memo.attachments
      .map((attachment) => {
        const imageUrl = attachment.externalLink || attachment.url;
        return `<img src="${imageUrl}" alt="资源图片" class="memo-resource">`;
      })
      .join("")}</div>`;
  }
...
// 当前选中的说说类型
let currentMemoType = "memos"; // 'memos' 或 'tgtalk'
...
async function renderMemos() {
  try {
    ...
    const tabsHTML = `
      ...
      <div class="memos-content-wrapper">
        <div class="memos-content" id="memos-content">
          <!-- 内容将在这里动态加载 -->
        </div>
      </div>
    `;
...
// 切换说说类型
async function switchMemoType(type) {
  if (type === currentMemoType) return;

  currentMemoType = type;

  // 更新 Tab 样式
  document.querySelectorAll(".memos-tab").forEach((tab) => {
    tab.classList.remove("active");
  ...
  try {
    ...
    const pinnedMemosHTML =
      pinnedMemos.length > 0
        ? `<div class="pinned-memos-section">${pinnedMemos.map(createMemoHTML).join("")}</div>`
        : "";

    // 初始显示的非置顶说说
    const initialMemosHTML = allUnpinnedMemos
    ...
    const loadMoreButton = hasMoreMemos
      ? `<div class="load-more-memos">
          <button class="load-more-button" onclick="loadMoreMemos()">加载更多</button>
        </div>`
      : "";
    ...
    console.error("Error rendering memos content:", error);
    container.innerHTML = `
      <div class="blankslate">
        <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
          <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
...
async function renderTgtalkContent(container) {
  try {
    ...
    console.error("Error rendering tgtalk content:", error);
    container.innerHTML = `
      <div class="blankslate">
        <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
          <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
...
function loadMoreMemos() {
  ...
  const nextBatchEnd = Math.min(
    currentDisplayCount + MEMOS_PER_LOAD,
    allUnpinnedMemos.length,
  );
...
function initLightbox() {
  ...
  const tgtalkImages = document.querySelectorAll(".tgtalk-resources img");

  // 如果没有图片需要灯箱功能，直接返回
  if (
    memoImages.length === 0 &&
    readmeImages.length === 0 &&
    tgtalkImages.length === 0
  ) {
    return;
  }
  ...
  const allImages = document.querySelectorAll(
    ".memo-resources img, .readme-content img, .tgtalk-resources img",
  );
  ...
  // 为 tgtalk 说说单独初始化
  const tgtalkMemos = document.querySelectorAll(".tgtalk-resources");
  tgtalkMemos.forEach((memo) => {
    const images = memo.querySelectorAll("img");
    if (images.length > 0) {
      const imageUrls = Array.from(images).map((img) => img.src);
      images.forEach((img) => {
...
async function showArticleDetail(article) {
  ...
  if (!articleModal) {
    ...
    articleModal
      .querySelector(".article-modal-close")
  ...
  articleModal.querySelector(".article-modal-title").textContent =
    article.title;
                    ...
                    ${
                      changelog.tags
                        ? changelog.tags
                            ...
                            .map(
                              (tag) =>
                                `<span class="changelog-tag">${tag.trim()}</span>`,
                            )
                            .join("")
                        : ""
                    }
...
function addTouchFeedback() {
  ...
  const touchButtons = document.querySelectorAll(
    ".show-more-button, .activity-action-btn",
  );
...
const originalCreateShowMoreButton =
...
function toggleCommitMessages(toggleElement, remainingCount) {
  ...
  const hiddenMessages = commitMessagesContainer.querySelector(
